...
public class WSEpec implements DFLog {
	...
	NFEnviaEventoEpecRetorno enviaEpec(NFLoteEnvio loteEnvio) throws Exception {
		...
		final NFEnviaEventoEpec epecAssinado = this.config.getPersister().read(NFEnviaEventoEpec.class,
				documentoAssinado);
	...
	private NFEnviaEventoEpec criaEnvioEpec(NFLoteEnvio loteEnvio) {
		...
		nfEnviaEventoEpec
				.setIdLote(StringUtils.isBlank(loteEnvio.getIdLote()) ? String.valueOf(new java.util.Date().getTime())
						: loteEnvio.getIdLote());
		...
		for (NFNota nfNota : loteEnvio.getNotas()) {
			...
			nfInfoEpec.setDataHoraEmissao(ZonedDateTime.ofInstant(
					nfNota.getInfo().getIdentificacao().getDataHoraEmissao().toInstant(), ZoneId.systemDefault()));
			...
			nfInfoEpec.setOrgaoAutor(DFUnidadeFederativa
	...
	public NFeRecepcaoEvento4Stub.NfeResultMsg comunicaLoteRaw(String loteAssinadoXml, DFModelo modelo)
			throws Exception {
		// valida o epec assinado, para verificar se o xsd foi satisfeito, antes de
		...
		final String endpoint = DFModelo.NFE.equals(modelo) ? autorizador.getRecepcaoEvento(this.config.getAmbiente())
