...
public abstract class DFCadeiaCertificados implements DFLog {
    ...
    public static byte[] geraCadeiaCertificados(final DFAmbiente ambiente, final String senha) throws Exception {
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (final NFAutorizador400 aut : NFAutorizador400.values()) {
                DFCadeiaCertificados.get(keyStore, aut.getNfeStatusServico(ambiente));
                DFCadeiaCertificados.get(keyStore, aut.getNfceStatusServico(ambiente));
            }

            //URLs especificas do AN (www.fazenda.gov.br e www1.fazenda.gov.br)
            DFCadeiaCertificados.get(keyStore, NFAutorizador400.AN.getRecepcaoEventoAN(ambiente));
            DFCadeiaCertificados.get(keyStore, NFAutorizador400.AN.getNFeDistribuicaoDFe(ambiente));

            // MDFE
            for (final MDFAutorizador3 aut : MDFAutorizador3.values()) {
                DFCadeiaCertificados.get(keyStore, aut.getMDFeStatusServico(ambiente));
            }

            // CTe
            for (final CTAutorizador31 aut : CTAutorizador31.values()) {
                DFCadeiaCertificados.get(keyStore, aut.getCteStatusServico(ambiente));
            }
    ...
    private static void get(final KeyStore keyStore, final String url) throws Exception {
        DFLog.getLogger(DFCadeiaCertificados.class).debug("Obtendo cadeia de certificados para a URL: {}", url);
