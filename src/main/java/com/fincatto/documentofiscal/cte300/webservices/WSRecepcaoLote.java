...
class WSRecepcaoLote implements DFLog {
	...
	public CTeEnvioLoteRetornoDados envioRecepcao(CTeEnvioLote cteRecepcaoLote) throws Exception {
		// assina o lote
		final String documentoAssinado = new DFAssinaturaDigital(this.config)
				.assinarDocumento(cteRecepcaoLote.toString(), "infCte");
	...
	private CTeEnvioLoteRetorno comunicaLote(final String loteAssinadoXml) throws Exception {
		...
		final CTAutorizador31 autorizador = CTAutorizador31.valueOfTipoEmissao(this.config.getTipoEmissao(),
				this.config.getCUF());
		final String endpoint = autorizador.getCteRecepcao(this.config.getAmbiente());
		if (endpoint == null) {
			throw new IllegalArgumentException("Nao foi possivel encontrar URL para Recepcao, autorizador "
					+ autorizador.name() + ", UF " + this.config.getCUF().name());
		}
		final CteRecepcaoLoteResult autorizacaoLoteResult = new CteRecepcaoStub(endpoint, config).cteRecepcaoLote(dados,
				cabecalhoSOAP);
		final CTeEnvioLoteRetorno retorno = this.config.getPersister().read(CTeEnvioLoteRetorno.class,
