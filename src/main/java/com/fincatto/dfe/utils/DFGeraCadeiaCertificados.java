...
public abstract class DFGeraCadeiaCertificados {

    private static final int PORT = 443;
    private static final String PROTOCOL = "TLS";
    private static final Logger LOGGER = LoggerFactory.getLogger(DFGeraCadeiaCertificados.class);
        ...
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            for (final NFAutorizador31 aut : NFAutorizador31.values()) {
                ...
                if (StringUtils.isNotBlank(urlNF)) {
                    ...
                    DFGeraCadeiaCertificados.get(keyStore, host, PORT);
                }
            ...
            for (final CTAutorizador autCte : CTAutorizador.values()) {
                //Para CTe...
                final String urlCT = autCte.getCteStatusServico(ambiente);
                if (StringUtils.isNotBlank(urlCT)) {
                    final String host = new URI(urlCT).getHost();
                    DFGeraCadeiaCertificados.get(keyStore, host, PORT);
                }
            }
