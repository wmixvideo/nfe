...
public class DFSocketFactory implements ProtocolSocketFactory {
    ...
    public DFSocketFactory(final DFeConfig config) throws KeyManagementException, UnrecoverableKeyException, KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {
        this.sslContext = this.createSSLContext(config);
    }

    private SSLContext createSSLContext(final DFeConfig config) throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException, KeyManagementException, UnrecoverableKeyException {
    ...
    private KeyManager[] createKeyManagers(final DFeConfig config) throws UnrecoverableKeyException, KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {
        try (InputStream certificado = new ByteArrayInputStream(config.getCertificado())) {
            ...
            final String alias = DFSocketFactory.getAlias(ks);
    ...
    private TrustManager[] createTrustManagers(final DFeConfig config) throws KeyStoreException, NoSuchAlgorithmException, CertificateException, IOException {
